...
class ValueNetwork(object):
    ...
    def step(self, obs):
        with tf.compat.v1.variable_scope(self.name):
            h1 = tf.compat.v1.layers.dense(obs, 128, tf.nn.leaky_relu, name='h1')
            h2 = tf.compat.v1.layers.dense(h1, 128, tf.nn.leaky_relu, name='h2')
            h3 = tf.compat.v1.layers.dense(h2, 128, tf.nn.leaky_relu, name='h3')
...
class QValueNetwork(object):
    ...
    def step(self, obs, action, reuse):
        with tf.compat.v1.variable_scope(self.name, reuse=reuse):
            ...
            h1 = tf.compat.v1.layers.dense(input, 128, tf.nn.leaky_relu, name='h1')
            h2 = tf.compat.v1.layers.dense(h1, 128, tf.nn.leaky_relu, name='h2')
            h3 = tf.compat.v1.layers.dense(h2, 128, tf.nn.leaky_relu, name='h3')
            # h4 = tf.compat.v1.layers.dense(h3, 1024, tf.nn.leaky_relu)
...
class ActorNetwork(object):
    ...
    def step(self, obs, log_std_min=-20, log_std_max=2):
        with tf.compat.v1.variable_scope(self.name, reuse=tf.compat.v1.AUTO_REUSE):
            ...
            log_std = tf.compat.v1.layers.dense(h3, self.act_dim, tf.tanh, name='log_std')
