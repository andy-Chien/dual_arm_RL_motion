...
class ValueNetwork(object):
    ...
    def step(self, obs):
        with tf.compat.v1.variable_scope(self.name):
            h1 = tf.compat.v1.layers.dense(obs, 512, tf.nn.leaky_relu, name='h1')
            h2 = tf.compat.v1.layers.dense(h1, 512, tf.nn.leaky_relu, name='h2')
            h3 = tf.compat.v1.layers.dense(h2, 512, tf.nn.leaky_relu, name='h3')
...
class QValueNetwork(object):
    ...
    def step(self, obs, action, reuse):
        with tf.compat.v1.variable_scope(self.name, reuse=reuse):
            ...
            h1 = tf.compat.v1.layers.dense(input, 512, tf.nn.leaky_relu, name='h1')
            h2 = tf.compat.v1.layers.dense(h1, 512, tf.nn.leaky_relu, name='h2')
            h3 = tf.compat.v1.layers.dense(h2, 512, tf.nn.leaky_relu, name='h3')
            # h4 = tf.compat.v1.layers.dense(h3, 1024, tf.nn.leaky_relu)
...
class ActorNetwork(object):
    ...
    def step(self, obs, log_std_min=-20, log_std_max=2):
        with tf.compat.v1.variable_scope(self.name, reuse=tf.compat.v1.AUTO_REUSE):
            ...
            h2 = tf.compat.v1.layers.dense(h1, 512, tf.nn.leaky_relu, name='h2')
            h3 = tf.compat.v1.layers.dense(h2, 512, tf.nn.leaky_relu, name='h3')
            h4 = tf.compat.v1.layers.dense(h3, 512, tf.nn.leaky_relu, name='h4')
            h5 = tf.compat.v1.layers.dense(h4, 512, tf.nn.leaky_relu, name='h5')
