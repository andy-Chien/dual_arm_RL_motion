...
def train(nameIndx):
    ...
    for i in range(MAX_EPISODES):
        ...
        print('Episode:', i, ' Reward: %i' % int(ep_reward), 'MU_REWARD: ', int(MU_REWARD),'BEST_R: ', int(BEST_R), 'cnt = ',j, 's_rate = ', int(SUCCESS_RATE))# , 't_step:', int(t23), 't_learn: ', int(t32)) #'var: %.3f' % var, 'rar: %.3f' % rar)
        if SAVE[nameIndx]:
            print(agent.path)
